...
def create_collection():
    try:
        response = httpx.post(f"{QDRANT_API_URL}/create-collection", timeout=60.0)
...
def upload_pdf(file):
    try:
        files = {"file": file}
        # Aumentar timeout para upload de PDF (pode demorar mais)
        response = httpx.post(f"{QDRANT_API_URL}/upload-pdf", files=files, timeout=120.0)
        response.raise_for_status()
        return response.json()
    except Exception as e:
